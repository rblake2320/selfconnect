...
| TPM/CNG key use | Proven in package path | `sc_tpm_attestation.py`, `tests/test_tpm_attestation.py`, `proofs/capability_os/tpm_user_scope_live_20260725.json`; the current identity-key path is user-scoped, hardware-backed, PCP-identity marked, non-exportable, and passed unelevated real-hardware quote verification |
