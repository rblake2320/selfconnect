...
def _case(tmp_path, *, birth="seat-a", now=1000.0):
    ...
    enrollment = create_enrollment(
        seat_identity=seat, authority_identity=authority, birth_id=birth, generation=1, now=now
    )
    issue_store = tmp_path / f"{birth}-issued.sqlite3"
    live_snapshot = {"ok": True, "tab_runtime_id": [7, 11], "term_control_runtime_id": [7, 19], "peer_birth_id": birth}
    ...
    delivery = deliver_challenge_postmessage(
        ...
        tab_checkpoint=lambda stage: checkpoints.append(stage) or {**live_snapshot, "stage": stage},
        now=now,
    )
    proof = create_proof(
        seat_identity=seat,
    ...
    kwargs = {
        ...
        "replay_store": tmp_path / f"{birth}.sqlite3",
        "issue_store": issue_store,
        "now": now + 1,
    }
...
def test_r0_verify_proof_rejects_enrollment_not_signed_by_pinned_authority(tmp_path):
    ...
    rogue_enrollment = create_enrollment(
        seat_identity=seat, authority_identity=rogue, birth_id="seat-a", generation=1, now=1000
    )
...
def test_unsigned_challenge_and_plus_five_second_forward_skew_rejected(tmp_path):
    *_prefix, proof, kwargs, _ = _case(tmp_path)
    ...
    delivery = deliver_challenge_postmessage(
        ...
        tab_checkpoint=lambda _stage: {
            "ok": True,
            "tab_runtime_id": [7, 11],
            "term_control_runtime_id": [7, 19],
            "peer_birth_id": "seat-a",
        },
        now=kwargs["now"],
    )
    future_proof = create_proof(
        seat_identity=_prefix[2],
        challenge=future,
        delivery=delivery,
        authority_public_key_hex=authority.public_key_hex,
        issue_store=future_store,
    ...
    with pytest.raises(ValueError, match="not currently valid"):
        verify_proof(future_proof, **{**kwargs, "challenge": future, "delivery": delivery, "issue_store": future_store})
