...
def test_runtime_verifier_rejects_caller_supplied_evidence_dict():
    with pytest.raises(TypeError, match="pipe-issued"):
        verify_proof_runtime(
...
def test_pipe_receipt_has_no_public_constructor():
    with pytest.raises(TypeError, match="only be issued"):
        PipeIssuedEvidence("caller", {}, {})
    forged = object.__new__(PipeIssuedEvidence)
    with pytest.raises(TypeError, match="pipe-issued"):
        _open_pipe_receipt(forged, {})
...
def test_live_one_shot_pipe_derives_sid_pid_and_process_start(tmp_path):
    ...
    proof, issued_evidence = _open_pipe_receipt(result["receipt"], challenge)
...
@pytest.mark.skipif(os.name != "nt", reason="real Win32 named-pipe proof")
def test_live_pipe_receipt_drives_production_runtime_verifier(tmp_path):
    authority = AgentIdentity.generate("authority")
    recovery = AgentIdentity.generate("recovery")
    receiver_identity = AgentIdentity.generate("receiver")
    seat = AgentIdentity.generate("seat")
    trust = bootstrap_authority_trust(
